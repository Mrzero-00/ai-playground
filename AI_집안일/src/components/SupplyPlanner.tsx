    ...
    {!items.length ? <div className="report-empty">샴푸, 휴지, 세제처럼 미리 챙겨야 하는 품목을 등록해보세요.</div> : <>
      ...
      {selectedItem && dashboard && <div className="supply-dashboard">
        ...
        <div className="supply-metric-grid">
          <article className="supply-usage-card"><span>주간 평균 사용량</span><strong>{selectedItem.weeklyUsage}<small>{selectedItem.unit}</small></strong><div className="supply-rate-meter" aria-label={`구매 수량 중 주간 사용 비율 ${Math.min(100, Math.round((selectedItem.weeklyUsage / selectedItem.purchaseQuantity) * 100))}%`}><i style={{ width: `${Math.min(100, (selectedItem.weeklyUsage / selectedItem.purchaseQuantity) * 100)}%` }} /></div><p>직접 입력한 평균값으로 계산해요.</p></article>
