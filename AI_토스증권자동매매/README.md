...
Runner는 미국 시장 거래대금 상위와 1일 급등 랭킹의 교집합을 찾고, 거래대금·등락률·시세 신선도와 2회 연속 상승 관측을 통과한 종목만 검증합니다. 이후 무료 Google News RSS의 최근 48시간 기사와 SEC EDGAR의 최근 주요 공시를 병렬로 확인하고, Bull·Bear·Market·Skeptic 독립 에이전트 위원회의 합의를 통과한 때만 가상 진입합니다. 판단과 근거는 상태 파일에 저장되며 OpenAI API는 사용하지 않습니다. 진입과 청산에는 슬리피지와 주문별 수수료를 반영하며 1차 부분 익절, 2차 익절, 가격 손절, 시간 손절을 처리합니다.

`.env`의 `SEC_USER_AGENT`는 실제 이메일을 포함한 식별 문구로 설정하는 것을 권장합니다. 뉴스·공시는 기본 15분간 캐시됩니다.
