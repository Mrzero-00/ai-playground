...
1. 3초 Countdown 동안 차량이 움직이지 않는지 확인한다.
2. `GO!` 뒤 Gas를 눌러 오른쪽으로 출발한다.
3. 바닥 중앙선을 따라 파란 라인을 `CP 1 → 2 → ... → 13` 순서대로 통과한다.
4. 출발 지점의 노란 Finish 라인으로 돌아온다.
5. 결과가 `FINISH` 또는 `NEW BEST`와 `mm:ss.mmm` 전체 Race Time으로 표시되는지 확인한다.
6. 오른쪽 `LOCAL TOP 5`의 1위에 같은 기록이 저장되는지 확인한다.
...
4. Ghost와 충돌하지 않고 저장된 주행을 부드럽게 따라가는지 확인한다.
