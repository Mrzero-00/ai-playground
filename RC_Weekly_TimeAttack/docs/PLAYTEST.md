...
2. 노출된 타이어를 포함한 외형과 실제 벽 충돌 위치가 자연스럽게 일치하는지 확인한다.
3. 3초 Countdown 동안 차량이 움직이지 않는지 확인한다.
4. `GO!` 뒤 Gas를 눌러 오른쪽으로 출발한다.
5. 바닥 중앙선을 따라 파란 라인을 `CP 1 → 2 → ... → 13` 순서대로 통과한다.
6. 출발 지점의 노란 Finish 라인으로 돌아온다.
7. 결과가 `FINISH` 또는 `NEW BEST`와 `mm:ss.mmm` 전체 Race Time으로 표시되는지 확인한다.
8. 오른쪽 `LOCAL TOP 5`의 1위에 같은 기록이 저장되는지 확인한다.
...
4. Ghost가 플레이어 차량과 같은 미니카 크기이며 충돌하지 않고 저장된 주행을 부드럽게 따라가는지 확인한다.
