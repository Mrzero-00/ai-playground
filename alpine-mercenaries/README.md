...
첫 무기 세트로 검과 방패, 장궁, 대검을 추가했다. 좌클릭 기본 공격은 무기별 3단 콤보이며 마지막 성공 입력 후 1.25초가 지나면 1타로 돌아간다. 우클릭은 검방의 유지형 가드, 활의 누르고 조준한 뒤 떼어 발사하는 집중 사격, 대검의 누르고 충전한 뒤 떼어 사용하는 충전 베기다. 숫자 `1`~`3`에는 현재 무기 전용 특수공격 슬롯을 마련했으며, 이후 정식 스킬 데이터·애니메이션·효과로 확장한다.

던전에 입장하면 출발 전에 장착한 무기를 끝까지 사용하며 전투 중 정식 무기 교체는 제공하지 않는다. 현재 화면 하단의 세 무기 버튼은 개발 검증 전용이다. `Tab`으로 임시 선택 커서를 열고 버튼을 클릭할 때만 무기를 바꿀 수 있다. 장착·콤보·특수공격·역할 행동 상태와 서버 요청 경로는 복제 가능한 컴포넌트로 분리했으며, 현재 무기 외형과 휘두르기는 기능 검증용 절차적 임시 표현이다. 최종 모델·손 위치·전투 애니메이션은 전투 대상과 함께 교체한다.

기본 맵에는 플레이어 전방에 움직이지 않는 훈련 허수아비가 한 개 자동 생성된다. 모든 무기의 근접·원거리 피해를 실제 서버 피해 경로로 받고, 머리 위에 현재 HP·최근 피해·누적 타격 횟수를 표시한다. HP 300을 모두 소진하면 4초 뒤 자동으로 복구되어 공격 피해와 콤보를 반복 확인할 수 있다.

허수아비 오른쪽 뒤에는 플레이어를 향해 4초마다 피해 20의 발광 투사체를 발사하는 개발용 장치가 함께 생성된다. 검과 방패를 장착하고 장치를 바라본 상태에서 우클릭을 유지하면 정면 투사체 피해가 0이 되고 HUD의 `SHIELD BLOCKED` 횟수가 증가한다. 방패를 내리거나 장치를 등지면 투사체가 HP를 감소시키므로 방향 방어를 반복 비교할 수 있다.
...
이동·자원·무기·훈련 허수아비·투사체 방어 설정과 입력·게임 모드 연결 자동화 테스트는 다음을 실행한다. 스크립트는 C++ 에디터 모듈을 먼저 빌드한 뒤 `NullRHI` 헤드리스 모드로 실행하므로 게임 창을 열지 않는다.
...
기본 키보드·마우스 조작은 WASD 이동, 마우스 시점, `Left Shift` 질주, `Left Control` 걷기, `C` 앉기 전환, `Space` 점프, `Q` 좌·우 어깨 전환, `Home` 기본 숄더 시점 복귀다. 좌클릭은 무기별 3단 기본 콤보, 우클릭을 누르고 있는 동안 무기 고유 행동, 숫자 `1`~`3`은 현재 무기의 특수공격 슬롯이다. 개발 중에만 `Tab`을 눌러 하단 임시 무기 선택 버튼을 클릭할 수 있다.
