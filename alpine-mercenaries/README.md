...
- 검과 방패·활·대검의 장착, 교체, 공격 판정과 역할 행동 기반
...
- 현재 검증: C++ 캐릭터에서 이동·카메라·충돌·애니메이션·무기 행동 확인
...
첫 무기 세트로 검과 방패, 장궁, 대검을 추가했다. 검방은 전방 가드 영역, 활은 정밀 조준과 원거리 선형 판정, 대검은 넓은 근접 판정과 충전 공격을 가진다. 장착·행동 상태와 서버 요청 경로를 복제 가능한 컴포넌트로 분리했으며, 현재 무기 외형은 기능 검증용 절차적 임시 모델이다. 최종 모델·손 위치·전투 애니메이션은 전투 대상과 함께 교체한다.
...
이동·자원·무기 설정과 입력·게임 모드 연결 자동화 테스트는 다음을 실행한다. 스크립트는 C++ 에디터 모듈을 먼저 빌드한 뒤 `NullRHI` 헤드리스 모드로 실행하므로 게임 창을 열지 않는다.
...
기본 키보드·마우스 조작은 WASD 이동, 마우스 시점, `Left Shift` 질주, `Left Control` 걷기, `C` 앉기 전환, `Space` 점프, `Q` 좌·우 어깨 전환, `Home` 기본 숄더 시점 복귀다. 무기는 `1` 검과 방패, `2` 활, `3` 대검으로 교체하며 좌클릭은 기본 공격, 우클릭을 누르고 있는 동안 무기별 역할 행동을 사용한다.
