...
7. 걷기, 질주, 앉기 캡슐과 점프를 구현한다. 완료. 전용 앉기 포즈와 낮은 장애물 넘기는 미완료.
...
현재 작업은 기본 이동 게이트에만 집중한다. 전용 앉기 자세와 앉은 이동, 점프·낙하·착지, 낮은 장애물 넘기·창문 통과·턱 오르기, 슬라이딩을 충돌·카메라·스태미너·애니메이션까지 함께 검증한다.

기본 이동이 완료되면 공통 무기 기반과 검방·활·대검, 첫 몬스터 기반, 몬스터의 휘두르기·돌진·브레스, 방향 방어와 아군 보호 모션, 활의 부위별 약점 공격 순서로 진행한다. 뒤 단계 기능을 먼저 넣어 기본 이동의 미완료 상태를 가리지 않는다.
...
현재 앉기 캡슐과 카메라 높이는 전환되지만 기본 Animation Blueprint에 전용 앉기 포즈가 없어 애니메이션 일치 항목은 아직 완료하지 않았다.
