...
## 2026-07-23 방패 방어용 투사체

- 자동 생성: 훈련 허수아비와 궤적이 겹치지 않는 플레이어 전방 우측에 정지형 발사 장치 한 개 생성
- 공격: 생성 3초 후 첫 발, 이후 4초마다 속도 700cm/s·피해 20의 발광 직선 투사체 발사
- 피해 경로: 서버 `ApplyPointDamage`를 캐릭터 `TakeDamage`와 HP 컴포넌트에 연결
- 방어: 검방 우클릭 가드가 활성화되고 피해 발생 지점이 카메라 전방 120도·250cm 안이면 피해 0
- 실패 조건: 가드 해제, 측·후면 피격 또는 다른 무기 장착 시 HP 피해 적용
- 피드백: HUD에 누적 방어 횟수와 마지막으로 막은 피해 표시
- 자동화: `AlpineMercenaries.Combat.ProjectileDefense`를 추가한 5개 헤드리스 테스트
