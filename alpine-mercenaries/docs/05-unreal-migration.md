...
- 네이티브 프로젝트: 공식 Third Person Blueprint 템플릿 기반으로 생성 완료
- Git 대용량 파일: Git LFS 3.7.1 활성화
- Xcode: Command Line Tools만 설치됨. UE 5.8 권장 Xcode 26.1.1 전체 설치 필요
...
4. Xcode 26.1.1 전체 앱을 설치하고 `xcode-select` 개발자 경로를 확인한다. UE 5.8과 호환되지 않는 26.4는 사용하지 않는다.
5. 프로젝트를 Unreal Editor에서 열고 기본 맵을 Play-In-Editor로 검증한다.
6. 배틀그라운드형 숄더 카메라를 프로젝트 고유 카메라로 구현한다.
7. 걷기, 질주, 앉기, 점프와 낮은 장애물 넘기를 순서대로 구현한다.
8. Unreal 게임플레이 자동화 테스트와 네이티브 개발 빌드를 실행한다.
...
- 프로젝트 에셋 Data Validation: 171개 검사, 오류 0건
- 캐릭터 메시, 애니메이션, Enhanced Input과 레벨 프로토타이핑 참조: 로드 통과
- 남은 엔진 경고: Mac Metal 툴체인 없음 1건
- 미검증: Play-In-Editor 조작, 프로젝트 고유 숄더 카메라, 질주·앉기·파쿠르, Mac 패키징

반복 검증 명령은 `scripts/verify-unreal-project.sh`다. 기본 설치 위치가 다르면 `AM_UNREAL_EDITOR_CMD`에 `UnrealEditor-Cmd` 절대 경로를 지정한다.
