...
- 공통 시스템: `AlpineWeaponComponent`가 입장 시 고정 장착 무기, 역할, 콤보·특수공격 상태, 스태미너, 재사용 대기시간과 히트 수를 관리
- 검과 방패: 기본 근접 스윕과 전방 가드 영역·유지 스태미너 구현
- 활: 카메라 방향 원거리 라인 트레이스, 정밀 조준 카메라와 피해 증폭 구현
- 대검: 넓은 근접 스윕과 유지형 충전·다음 공격 피해 증폭 구현
- 입력: 좌클릭 무기별 3단 콤보, 우클릭 검방 유지형 가드·활 조준 후 떼어 집중 사격·대검 충전 후 떼어 충전 베기, 숫자 `1`~`3` 특수공격 슬롯과 게임패드 대응
- 콤보: 검방 사선베기·횡베기·찌르기를 포함한 무기별 3단 정의와 1.25초 초기화
- 장착 규칙: 던전 중 교체 입력 없음. `Tab`으로 여는 화면 하단 검방·활·대검 버튼만 개발 검증 목적으로 임시 교체 허용
- 네트워크 기반: 서버 RPC와 장착·행동 상태 복제 필드 추가
- 표현: 캐릭터 손 뼈에 부착되는 기능 검증용 임시 무기 외형, 콤보별 절차적 움직임과 HUD 상태·스킬 슬롯 표시
- 자동화: `AlpineMercenaries.Weapons.Foundation`을 포함한 당시 3개 테스트를 GUI 없이 통과

## 2026-07-23 정지형 피해 검증 대상

- 자동 생성: 게임 모드가 첫 플레이어 전방 약 3m의 바닥에 훈련 허수아비 한 개만 생성
- 피해 경로: 무기 시스템의 서버 `ApplyPointDamage`를 그대로 받아 HP 300에서 실제 피해 차감
- 피드백: 월드 공간 HP 막대, 현재 HP·최근 피해·누적 타격 횟수와 체력별 색상 표시
- 반복 검증: HP 0에서 4초 후 자동 복구하며 이동·공격·Tick 없음
- 네트워크 기반: HP·최근 피해·타격 횟수 복제와 서버 권위 피해·리셋
- 자동화: `AlpineMercenaries.Combat.TrainingTarget`을 추가한 4개 헤드리스 테스트
