...
- [x] `pnpm build`가 `out/`과 `cat-mbti-00.ait`를 생성한다.
- [ ] App in Toss 테스트 환경에서 새로고침·직접 URL 진입이 정상인지 확인한다.
- [x] App in Toss Web Framework/CLI 3.1.1을 설치하고 현재 공식 방식으로 `.ait` 번들을 생성한다.
- [x] `.ait` 번들의 **압축 해제 기준 용량이 100MB 이하**인지 측정한다. 2026-08-31 기준 61개 파일, 약 14.3MB다.
- [x] 실행에 꼭 필요한 자산만 번들에 포함하고 캐릭터 이미지를 최적화한다.

비게임 출시 가이드는 SSR을 허용하지 않고 CSR 또는 SSG를 요구한다. Next.js를 유지하더라도 서버 런타임 없이 완결되는 정적 앱이어야 한다.

## 3. 콘솔과 SDK 설정

- [x] 콘솔의 기존 `고양이 MBTI` 미니앱과 정확한 `appName` `cat-mbti-00`을 확인해 사용한다.
- [x] SDK 3.x의 `apps-in-toss.config.ts`에 브랜드 색 `#FF765F`, 정적 번들 경로, 내비게이션과 WebView 옵션을 설정한다.
- [x] 비게임 정적 WebView 앱으로 구성한다.
- [x] 2026-08-31 기준 최신 SDK major와 현재 설정 스키마를 확인한다.
...
- [x] App in Toss 비게임 네이티브 내비게이션 바를 설정한다.
- [x] `navigationBar.withBackButton`을 켠다.
...
- [x] 토스 런타임에서는 자체 `AppHeader`를 숨겨 네이티브 뒤로가기와 중복되지 않게 한다.
- [ ] `/profile`, 문항 중간, 마지막 문항, `/result`에서 뒤로가기 흐름을 각각 검증한다.
- [ ] 실제 토스 앱에서 첫 화면의 네이티브 뒤로가기가 미니앱을 종료하는지 확인한다. 기본 동작이 다르면 준비된 SDK `Screen.close()` adapter를 호출한다.
...
- [x] SDK 3.x `SafeArea.get()`으로 최초 상·우·하·좌 여백을 읽는다.
- [x] `SafeArea.subscribe()`로 inset 변경을 반영한다.
- [x] inset 값을 CSS 변수로 전달해 화면 패딩에 적용한다.
...
현재 설치한 SDK 3.1.1에서는 `SafeArea.get()`과 `SafeArea.subscribe()`를 사용한다. 과거 `SafeAreaInsets` 명칭을 그대로 복사하지 말고 설치 버전의 타입 정의와 공식 문서를 따른다.

## 6. 보안과 서비스 정책

- [x] `eval`, 원격에서 받은 코드 실행, 동적 스크립트 주입을 사용하지 않는다.
- [x] 외부 자사 사이트 강제 이동이나 브라우저 히스토리 우회 로직을 사용하지 않는다.
- [x] 현재 MVP는 외부 API·WebSocket을 사용하지 않는다.
