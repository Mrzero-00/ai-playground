...
- [x] `pnpm build`가 `out/`을 생성한다. 2026-08-25 최종 빌드 기준 약 14MB다.
- [ ] App in Toss 테스트 환경에서 새로고침·직접 URL 진입이 정상인지 확인한다.
- [x] 업로드 최소 요건인 SDK 2.x 계열의 `@apps-in-toss/web-framework` 2.10.8과 `ait build` 스캐폴딩을 설치했다.
- [ ] npm 최신 3.1.1로의 major 업그레이드는 제출 필수가 아니므로 자동 적용하지 않았다. 실제 콘솔의 권장 버전과 3.x 마이그레이션 문서를 확인한 뒤 별도 회귀 테스트로 결정한다.
- [ ] 실제 콘솔 값을 `.env.ait`에 넣고 `pnpm build:ait`로 `.ait` 번들을 생성한다.
- [x] 정적 산출물 `out/`은 약 14MB로 100MB 기준보다 충분히 작다.
- [ ] 실제 콘솔 값으로 만든 `.ait` 번들의 **압축 해제 기준 용량이 100MB 이하**인지 최종 측정한다.
...
```bash
cp .env.example .env.ait
# .env.ait에 콘솔에서 발급받은 실제 값을 입력
pnpm build:ait
```

`.env.ait`는 `.gitignore`의 `.env*` 규칙으로 커밋되지 않는다. `AIT_APP_NAME`, `AIT_DISPLAY_NAME`, `AIT_ICON_URL`이 비어 있으면 빌드는 CLI 실행 전에 종료된다. 일반 웹 개발은 이 값 없이 `pnpm dev`, `pnpm build`로 계속 가능하다.

- [ ] 콘솔에 냥BTI 미니앱을 등록하고 발급된 정확한 `appName`을 사용한다.
- [x] `granite.config.ts`를 추가하고 `appName`, 표시명, 로고 URL을 환경변수 필수값으로 두었다.
- [x] WebView 유형을 비게임용 `partner`로 설정했다.
- [x] SDK 2.x 마이그레이션 안내에 따라 `granite build`가 아닌 `ait build`를 사용한다.
- [ ] SDK 기능별 문서의 최소 버전은 패키지 전체의 최신 권장 버전과 같다고 가정하지 않는다.
- [ ] 권한은 실제 필요한 항목만 선언한다. 현재 냥BTI MVP는 기본 설문 흐름에 네이티브 권한이 필요하지 않다.

2026-03-23부터 SDK 1.x로 빌드한 버전은 업로드할 수 없다. 이 프로젝트는 업로드 가능한 2.x 계열의 2.10.8을 정확히 고정했다. 2026-08-25 npm 조회 기준 최신은 3.1.1이지만, major 업그레이드는 별도 마이그레이션과 실기기 회귀 테스트 후 진행한다.

## 4. 내비게이션과 뒤로가기

`adapters/webview.ts`에는 SDK 런타임 판별과 `graniteEvent`/`closeView` fallback 함수가 준비되어 있지만 현재 화면의 호출처는 없다. 토스 안에서는 네이티브 내비게이션을 사용하도록 자체 `AppHeader`를 숨긴다. 첫 화면 종료와 내부 뒤로가기는 QR 환경의 기본 동작을 우선 확인하고, 실제로 커스텀 처리가 필요한 경우에만 adapter를 연결한다.

- [x] `partner` 타입과 토스 런타임 감지를 적용하고, 토스 안에서는 자체 헤더를 숨겨 중복 내비게이션을 방지한다.
- [ ] App in Toss 비게임 내비게이션 바가 QR 실기기에서 정상 노출되는지 확인한다.
...
- [x] 토스 런타임에서 자체 `AppHeader`를 숨기도록 구현했다.
- [ ] 토스 네이티브 뒤로가기와 자체 `AppHeader`가 QR 실기기에서 동시에 노출되지 않는지 확인한다.
- [ ] `/profile`, 문항 중간, 마지막 문항, `/result`에서 뒤로가기 흐름을 각각 검증한다.
- [ ] 첫 화면에서 뒤로가기를 누르면 미니앱이 종료된다.
- [ ] QR 테스트에서 기본 뒤로가기/종료가 요구사항을 만족하지 않을 때만 `subscribeBack`·`closeView`를 실제 화면 흐름에 연결한다.
...
- [x] `SafeAreaInsets.get()`으로 최초 상·우·하·좌 여백을 읽는 adapter를 연결했다.
- [x] `SafeAreaInsets.subscribe()`로 inset 변경을 구독하고 언마운트 시 해제한다.
- [x] inset을 `--ait-safe-top/right/bottom/left` CSS 변수로 전달하는 최상위 provider를 연결했다.
...
- [x] 2026-08-25 소스 검사에서 `eval`, `new Function`, 동적 스크립트 주입을 발견하지 않았다.
- [x] 외부 사이트로 보내는 `window.location.replace` 흐름이 없다.
- [x] 현재 서버 API와 WebSocket을 사용하지 않아 HTTP/WS 혼합 콘텐츠가 없다.
...
- [x] 냥BTI, 집사 궁합, 고양이 간 조화에서 과학적 검사·합사 성공·수의학적 진단을 단정하지 않는다.
- [x] 결과와 생활 궁합 리포트가 질병·서열·합사 성공 여부를 확정하지 않고, 우려되는 변화에는 전문가 상담을 안내한다.
...
- [x] 일반 브라우저는 noop, 지원되는 토스 런타임은 Toss Ads adapter를 선택한다.
...
- [x] 테스트 ID를 소스에 하드코딩하지 않는다. `test`와 `production` 모두 환경변수로 광고 ID를 주입하고, 운영 빌드는 `ait.v2.live.` ID가 아니면 빌드 검증에서 중단한다.
- [x] 일반 브라우저와 미지원 토스 앱에서는 안전한 `noop`으로 동작한다.
- [ ] `isSupported()`와 토스 앱 버전을 모두 확인한다.

### 배너

- [x] `TossAds.initialize` 완료 뒤에만 `attachBanner`를 호출한다.
- [x] 컴포넌트 언마운트에서 `destroy()`를 호출한다.
- [x] `unsupported`, `onNoFill`, 초기화/렌더 실패에서 컨테이너를 접어 빈 영역을 남기지 않는다.
...
- [ ] 로고: **600×600 PNG**, 불투명 배경, 자체 권리 보유 이미지로 준비한다.
- [ ] 썸네일: **1932×828 PNG**, 핵심 기능을 보여주는 고화질 이미지로 준비한다.
- [ ] 선택 스크린샷: 세로 **636×1048 PNG 최소 3장** 또는 가로 **1504×741 PNG 최소 1장**을 준비한다.
- [ ] 부제에는 비속어와 느낌표를 쓰지 않고, 상세 설명에는 접속→행동→결과 흐름을 적는다.
- [ ] 고객센터 이메일·연락처 또는 채팅 주소를 입력한다.
