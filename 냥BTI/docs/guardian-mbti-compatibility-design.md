...
- 조정 문구는 `humanValue - catNeed`의 부호를 반영한다. 집사 쪽 생활 성향이 더 강한 경우와 고양이 요구가 더 큰 경우에 서로 다른 안내를 제공한다.

### 6.1 제목

```text
{best 차원의 화면 라벨}이 가장 가까운 조합
```

예: `혼자만의 시간이 가장 가까운 조합`

### 6.2 가장 가까운 점

`best` 차원의 사람 값과 고양이 요구 값 평균을 낮음·중간·높음으로 나누어 문구를 선택한다. 예를 들어 `interaction`이 둘 다 낮으면 조용한 거리 유지가 가깝다고 설명하고, 둘 다 높으면 잦은 애정 표현과 교류 요구가 가깝다고 설명한다. 가장 작은 차이도 30을 넘으면 `잘 맞는다`고 표현하지 않고, 다섯 항목 중 차이가 가장 작지만 간격이 남아 있다고 안내한다.

### 6.3 맞춰주면 좋은 점

`needsCare` 차원은 차이의 방향에 따라 분기한다.

- 집사 값이 더 높음: 애정 표현·놀이 자극·규칙성·독립 시간·변화 조정이 고양이 요구보다 많거나 강할 수 있다는 관점에서 강도를 낮추거나 선택권을 주도록 안내한다.
- 고양이 요구가 더 높음: 고양이가 집사의 기본 생활 스타일보다 교류·놀이·예측 가능성·혼자 쉬는 시간·점진적 변화를 더 필요로 할 수 있다는 관점에서 필요한 시간을 추가하도록 안내한다.
- 차이가 16 미만: 큰 차이가 아니므로 새로운 문제를 단정하지 않고 현재 방식을 유지하며 그날의 반응을 보도록 안내한다.
...
- 가장 가까운 차원도 차이가 30을 넘으면 강한 일치 표현을 사용하지 않는다.
...
4. 집사 값이 높은 경우와 고양이 요구가 높은 경우에 서로 다른 조정 문구가 나온다.
5. 16개 MBTI와 6 Trait의 낮음·중간·높음 조합 전체에서 빈 문구나 비정상 값이 생기지 않는다.
...
- 가장 가까운 차원 자체의 차이가 큰 조합에 대한 별도 회귀 fixture 추가.
...
5. 점수 분포, MBTI별 평균·편향, 극단값을 실제 사용자 데이터 없이도 합성 fixture로 계속 QA한다.
