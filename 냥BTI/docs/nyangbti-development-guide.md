...
최종 코드는 `EI + NS + TF + JP` 순서다. `data/type-content.ts`는 캐릭터 이름·태그라인·색상 같은 유형 연출을 제공하고, 실제 설명·강점·주의점·관찰 신호·관리 문구는 `lib/result-copy.ts`가 6개 연속 Trait에서 생성한다. 같은 4글자라도 연속 점수가 다르면 설명이 달라진다.

네 축 중 `낮음` 단계가 3개 이상이면 한 유형의 특징을 강하게 단정하지 않고 `{CODE}에 가장 가까운 균형형`으로 표시한다. 각 축도 선택된 한 글자 대신 `E·I 균형`처럼 양쪽 방향을 함께 설명한다.
...
`lib/compatibility.ts`는 고양이의 6 Trait을 같은 5개 요구 프로필로 변환한 뒤 가중 절대거리를 계산한다. 점수는 결정적이며 45~97 범위로 제한한다. 가장 가까운 차원은 `가장 가까운 점`, 가장 먼 차원은 조정 팁을 만드는 데 사용한다. 조정 문구는 집사 값이 고양이 요구보다 높은지 낮은지도 구분해 반대 상황에 같은 조언이 나오지 않게 한다.
