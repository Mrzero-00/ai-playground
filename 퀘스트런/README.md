...
| 단계형 모험 지도                                           | 자동 전투 장면                                      |
| ---------------------------------------------------------- | --------------------------------------------------- |
| ![퀘스트런 단계형 모험](docs/preview-adventure-stages.png) | ![퀘스트런 자동 전투](docs/preview-auto-battle.png) |

`preview/index.html`은 단순 이미지가 아닌 실행형 브라우저 데모입니다. 러닝 시작 후
시간·거리·속도·경로가 실시간으로 변하고, 러닝 종료 보상이 퀘스트 진행도와 캐릭터
경험치, 전투 에너지에 반영됩니다. 하단 메뉴에서 퀘스트 보상, 몬스터 전투,
아이템 획득까지 직접 테스트할 수 있으며 상태는 브라우저에 자동 저장됩니다.
...
- 일일·주간·연속 달성 퀘스트
- 굽이진 길을 따라 순서대로 열리는 9단계 모험 스테이지
- 버튼 한 번으로 몬스터 처치까지 진행되는 자동사냥
- 루미의 돌진·공격 이펙트·피격·데미지 숫자를 보여주는 턴 전투 연출
- 스테이지별 몬스터·에너지 비용·골드·장비 보상
