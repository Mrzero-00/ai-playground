...
R1에서 `008_agent_orchestration_v1.sql`을 추가했다. 적용된 기존 Migration은 수정하지 않고 후속 변경은 새 Migration으로 추가한다.
...
- [x] Engine/Agent 권한 분리
- [x] Definition·Prompt·Provider·Schema Version 계보
- [x] Evidence-bound Claim
- [x] Point-in-time·Strategy Scope 검증
- [x] Deterministic Conflict 차단
- [x] DAG Plan·Run/Attempt 상태 계약
- [x] Retry·Fallback·Replay 계약

### 안전

- [x] Agent Domain 직접 쓰기 0건
- [x] Risk/Approval/Activation 권한 0건
- [x] Prompt Injection Tool 실행 0건
- [x] Secret 형태 Context/Output 필드 차단
- [x] 다른 사용자 Evidence 참조 0건
- [x] Agent 실패에 따른 위험 확대 0건

### API/DB

- [x] Definition/Plan/Run/Validation API
- [x] Audit/Outbox/Idempotency
- [x] RLS/Composite FK/Immutable Trigger
- [x] Result Hash/Replay

### 검증

- [x] Unit/Security/Invariant/Golden/Integration
- [x] 동일 Manifest Hash 일치
