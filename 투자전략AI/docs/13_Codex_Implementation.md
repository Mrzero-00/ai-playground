...
- 작성일: `2026-07-22`
- 최종 검토일: `2026-07-23`
- 명세 상태: `IMPLEMENTATION EVIDENCE BASELINE`
...
1. 01~12·14의 구현 상태를 동일한 기준으로 판정한다.
2. 코드 완료와 운영 준비 완료를 구분한다.
3. 구현 주장을 실제 파일·Migration·Test·Build 증거와 연결한다.
4. 자동 주문 기본 비활성, Human Approval, 실행 서비스 분리, 전략 분리 등 안전 불변식을 재확인한다.
...
| `FOUNDATION_IMPLEMENTED` | 타입·핵심 함수 또는 UI 골격이 있으나 선언된 R1 범위 검증은 미완료 |
| `R1_VERIFIED` | 문서에서 명시한 R1 범위의 Core/API/Schema·테스트·Migration 연결을 검증함 |
...
- 현재 Repository Baseline: **R1 VERIFIED**
...
`R1_VERIFIED`는 전체 목표 API나 운영 준비 완료를 뜻하지 않는다. R1 범위 밖 항목은 각 문서의 `R2+ TARGET`과 Open Gate에 명시하며, R1을 Production Ready로 표현하지 않는다.
...
| 14 | Automated Trading Execution | v1.3.0 | R1 Foundation Implemented | Intent, Preflight, Toss Adapter, In-process Idempotency, Reconciliation Schema |

03·04와 02의 명세 상태가 Draft인 사실을 숨기지 않는다. 현재 코드는 문서에 표시된 R1 범위를 구현·검증했으며 목표 API 전체와 Production Policy 확정은 별도 승인 대상이다.
...
| 자동 주문 기본 비활성·실행 경계 분리 | `automated-execution-v1`, `apps/execution` | Mode/Runtime/Toss Contract Test | PASS |
...
- R2+ Target API: Thesis 생성·Revision·Review, Human Stage Approval
...
- R2+ Target API: Report 목록·변화 비교·Delivery·Retry

### 6.11 11 UI/UX

- 상태: `R1 REVIEW MVP VERIFIED`
...
### 6.13 14 Automated Trading Execution

- 상태: `R1 FOUNDATION VERIFIED / LIVE HARD-BLOCKED`
- 코드: Execution Intent·Preflight·State Machine·DRY_RUN/PAPER·Toss OAuth/Order Adapter·분리 HTTP Service
- Schema: `013_automated_trading_execution_v1.sql`
- 증거: Core 8개, Execution Service·Runtime·Toss Adapter 14개 Test
- 안전 경계: 승인된 Decision·Proposal·Risk 계보, Account Allowlist, 승인 Notional 재계산, Price/Stale/Cash/Quantity Gate, In-process 멱등성, UNKNOWN, Kill Switch
- Open Gate: Main API Handoff, Preview Migration, 영속 Idempotency Ledger 연결, 다중 Instance Token 조정, 일반·조건주문 Read-only 대사, PAPER Forward Run, Security Review, LIVE Canary
...
apps/
  ...
  execution/              Isolated DRY_RUN/PAPER/LIVE service and Toss adapter
...
packages/core/src/
  ...
  automated-execution-v1/ Intent, preflight, state and idempotency contracts

supabase/migrations/      001~013 additive schema and RLS
docs/                     00~14 product and engineering baseline
```
...
Main API는 자동 주문을 실행하지 않는다. 승인된 Handoff 이후 별도 Execution Service만 Broker Port에 접근하며 Repository 기본 실행 모드는 `DRY_RUN`이다.
...
Evaluation
  ...
  -> Execution Intent / Preflight
  -> Isolated Broker Adapter
  -> Execution Record / Reconciliation
```

`Execution Record`는 승인된 외부 실행의 감사 기록 경계다. 14 Foundation은 승인된 Decision 이후 주문을 자동 제출할 수 있는 분리 경계와 Toss Adapter 계약을 구현했지만, R1 Runtime은 영속 승인 원장 연결 전까지 LIVE 시작을 Hard Block한다.
...
| Suite | 결과 |
|---|---:|
| Core Unit/Contract | 136 passed |
| API Contract | 11 passed |
| Web View Model | 5 passed |
| Execution Service/Toss Adapter | 14 passed |
| Verifier Regression | 10 passed |
| Product Code Total | 166 passed |
| Including Verifier | 176 passed |
| Document Quality | 251 checks passed |
| Implementation Verification | 483 checks passed |
| TypeScript | Core/API/Web/Execution passed |
| Production Build | Core/API/Next/Execution passed |
| Browser Review | Desktop + 390px Mobile 수동 확인 — Artifact 미보존 |
| Browser Console | 확인 시점 warning/error 0 — 자동 회귀 증거 아님 |

- Document Quality Result Hash: `f94d13d5c75e303d4a8a6a4ed5a6d9b1ec002be9162d64378860ee0db92a3b96`
- Implementation Verification Result Hash: `621a6284772dadd0e366b085bdef1a339318a0d277c23ab1dc625b50bc3515e5`
...
1. 01~12·14 Entry가 빠짐없이 고유해야 한다.
...
8. 00~14와 02 Part 문서의 공통 메타데이터가 완전해야 한다.
9. Markdown 로컬 링크·제목 계층·코드 Fence가 유효해야 한다.
10. 02 통합본과 5개 Part가 정확히 동기화되어야 한다.
11. R1 API와 R2+ Target API가 혼합 표시되지 않아야 한다.
12. 이미 완료된 작업을 다음 작업으로 되돌리는 오래된 시간축 표현이 없어야 한다.
...
- 자동 주문 기본 비활성, 실행 Process·Credential 분리, LIVE 다중 Gate
...
- 013 Migration의 실제 Supabase 적용과 토스 실계좌 Read-only 대사 없음
...
- Shadow 표본·기간 사전 등록
- Execution 영구 Repository·PAPER Forward Run·Kill Drill
...
1. 전체 Synthetic Walking Skeleton Replay를 통합 Test로 묶는다.
2. Preview Supabase에 001~013을 실제 적용한다.
3. Auth 사용자 A/B로 RLS E2E를 수행한다.
4. Backup/PITR와 Restore Drill을 수행한다.
5. 토스 계좌·Holdings·Order 조회를 Read-only로 연결해 대사한다.
6. Execution PAPER Forward Run과 Fault Injection을 수행한다.
7. Shadow 표본·기간·중단 기준을 사전 등록한다.

1은 Repository만으로 가능하다. 2 이후는 외부 환경과 Credential이 필요하다.
...
- [x] 01~12·14의 문서 Version과 구현 상태가 구분되어 있다.
...
- [x] 자동 주문 기본 비활성, Human Approval, 분리 Execution 경계를 재확인한다.
...
01~12·14의 **문서에 선언된 Repository R1 범위는 검증 완료**되었다. 이는 전체 목표 API나 운영 완료가 아니라 다음 범위만 의미한다.
...
- Manifest와 문서 품질을 합친 483개 문서·코드·실제 테스트 선언·Migration 연결 검사가 통과한다.

### 13 구현 완료 항목

- `implementation/status.manifest.json`에 01~12·14 Capability·Readiness·구현·테스트·Migration·Open Gate를 고정했다.
- `pnpm verify:implementation`이 Capability 누락, 경로 누락, Migration 순서·최신 Marker, Test Snapshot, README Link를 검사한다.
- `pnpm verify:docs`가 공통 메타데이터, 로컬 링크, 제목 계층, 미완성 표식, 02 정본 동기화와 API 상태·문서 시간축을 검사한다.
...
- Core/API/Web/Execution/Verifier 테스트 선언 수를 실제 파일에서 계산해 Snapshot 과장을 차단한다.
...
현재 Repository 안에서 남은 핵심 작업은 **Synthetic Walking Skeleton 통합 테스트**다. 그 다음 Breakpoint는 Preview Supabase·Auth/RLS·Provider Credential과 같은 **R2 외부 환경 증거의 부재**다.
